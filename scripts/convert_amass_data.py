...
import argparse
import cv2
from ase.poselib.poselib.skeleton.skeleton3d import SkeletonTree, SkeletonMotion, SkeletonState
...
if __name__ == "__main__":
    ...
    all_pkls = glob.glob("/home/jing/Documents/projs/AMASS_RAW/**/*.npz", recursive=True)
    # all_pkls = glob.glob("/home/jing/Documents/projs/AMASS_RAW/**/*.npz", recursive=True)
    ...
    length_acc = []
    for data_path in tqdm(all_pkls): # processing each data file
        bound = 0
        # splits = data_path.split("/")[7:]
        splits = data_path.split("/")[-3:]
        key_name_dump = "0-" + "_".join(splits).replace(".npz", "") # create a unique key
        
        if (not splits[0] in process_set): # check if the dataset belongs to the current processing set
            continue
        
        if key_name_dump in amass_occlusion: # Apply occlusion filters to exclude certain data based on predefined conditions.
        ...
        if not 'mocap_framerate' in  entry_data: # Check if the mocap_framerate key exists in the data. If not, skip the file.
            continue
        ...
        skip = int(framerate/30) # Calculate the skip factor to downsample the data to 30 FPS.
        ...
        smpl_local_robot.load_from_skeleton(betas=torch.from_numpy(beta[None,]), gender=gender_number, objs_info=None) # Load the robot from the given skeleton data.
        smpl_local_robot.write_xml(f"ase/data/assets/mjcf/{robot_cfg['model']}_humanoid.xml") # Write the robot configuration to an XML file.
        skeleton_tree = SkeletonTree.from_mjcf(f"ase/data/assets/mjcf/{robot_cfg['model']}_humanoid.xml") # Load a skeleton tree from the XML file.
        
        root_trans_offset = torch.from_numpy(root_trans) + skeleton_tree.local_translation[0] # Calculate the root translation offset and update the skeleton state.

        new_sk_state = SkeletonState.from_rotation_and_root_translation(
                    skeleton_tree,  # This is the wrong skeleton tree (location wise) here, but it's fine since we only use the parent relationship here. 
                    torch.from_numpy(pose_quat), # here the pose_quat is in mujoco order, local rotation
                    root_trans_offset,
                    is_local=True) # is_local is to indicate whether the input rotation is in global or local coordinates.
        
        if robot_cfg['upright_start']: # Convert the pose quaternions to global orientation if upright_start is set.
            ...
            pose_quat = new_sk_state.local_rotation.numpy() # Update the pose quaternions in the skeleton state.
    ...
    # import ipdb
    # ipdb.set_trace()
    dest_folder = "/home/jing/Documents/projs/amass/all_data"
    if not os.path.exists(dest_folder):
        os.makedirs(dest_folder)
    if upright_start:
        joblib.dump(amass_full_motion_dict, "/home/jing/Documents/projs/amass/all_data/amass_train_take6_upright.pkl", compress=True)
    else:
        joblib.dump(amass_full_motion_dict, "/home/jing/Documents/projs/amass/all_data/amass_train_take6.pkl", compress=True)
